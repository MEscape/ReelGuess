...
import { getStreakMultiplier } from '@/features/scoring/service'
...
export const StreakIndicator = memo(function StreakIndicator({ streak, size = 'md' }: StreakIndicatorProps) {
    ...
    // Multiplier for the NEXT correct vote (based on new streak = streak + 1)
    ...
    return (
        ...
        >
            ...
            {nextMultiplier > 1.0 && (
                ...
                >
                    (×{nextMultiplier % 1 === 0 ? nextMultiplier.toFixed(0) : nextMultiplier.toString().replace(/\.?0+$/, '')} next)
                </span>
            )}
        </div>
    )
