...
const DEFAULT_SETTINGS = { rounds_count: 7, timer_seconds: 60 } as const

/**
 * Creates a new lobby and inserts the host player in a single transaction
 * (two queries — Supabase does not support multi-table transactions over REST).
 *
 * @param hostName - Display name for the host player.
 * @returns The created lobby (with the host player included) and the player record.
 */
export function createLobby(
    ...
    return ResultAsync.fromPromise(
        (async () => {
            ...
            // 1. Insert lobby with a placeholder host_id (updated after player is created)
            ...
            if (lobbyError || !lobbyData) {
                throw { type: 'LOBBY_DATABASE_ERROR', message: lobbyError?.message ?? 'Failed to create lobby' } satisfies LobbyError
            }

            // 2. Insert the host player
            const { data: playerData, error: playerError } = await supabase
                .from('players')
                .insert({ lobby_id: code, display_name: hostName, avatar_seed: avatarSeed, is_host: true })
                .select()
                .single()

            if (playerError || !playerData) {
                throw { type: 'LOBBY_DATABASE_ERROR', message: playerError?.message ?? 'Failed to create host' } satisfies LobbyError
            }

            // 3. Patch lobby.host_id to the real player UUID
            ...
            if (updateError) {
                throw { type: 'LOBBY_DATABASE_ERROR', message: updateError.message } satisfies LobbyError
            }

            const player  = mapPlayerRow(playerData as unknown as PlayerRow)
    ...
    return ResultAsync.fromPromise(
        (async () => {
            ...
            const { data, error } = await supabase
                .from('players')
                .insert({ lobby_id: lobbyId, display_name: playerName, avatar_seed: avatarSeed, is_host: false })
                .select()
                .single()

            if (error || !data) {
                throw { type: 'LOBBY_DATABASE_ERROR', message: error?.message ?? 'Failed to add player' } satisfies LobbyError
            }
